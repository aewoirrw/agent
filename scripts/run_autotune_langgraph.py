...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run LangGraph-based Scientist-Executor-Critic closed-loop autotuning.")
    p.add_argument("--pocket", type=str, required=True, help="Path to pocket JSON (PocketInput)")
    p.add_argument("--out", type=str, default="autotune_outputs/langgraph_history.jsonl", help="Output JSONL path")
...
async def main() -> None:
    ...
    loop = LangGraphAutotuneLoop(scientist=scientist, executor=executor, critic=critic, config=cfg)
